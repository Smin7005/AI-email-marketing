...
export class CampaignService {
  ...
  async createCampaign(organizationId: string, data: CreateCampaignData) {
    ...
    try {
      ...
      if (manualEmail) {
        // For manual mode, we create a campaign without business IDs
        return await db.transaction(async (tx) => {
          ...
          // Create campaign item for manual entry with metadata
          await tx.insert(campaignItems).values({
            campaignId: campaign.id,
            businessId: null, // Manual entry doesn't have a business record
            status: 'pending',
            metadata: {
      ...
      // Start transaction for normal mode
      return await db.transaction(async (tx) => {
        // Create campaign
        const [campaign] = await tx
          .insert(campaigns)
  ...
  async getCampaignWithDetails(organizationId: string, campaignId: number) {
    ...
    // Get campaign items with business details or manual metadata
