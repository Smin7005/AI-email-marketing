...
export class CampaignServiceSupabase {
  ...
  async createCampaign(organizationId: string, data: CreateCampaignData) {
    const { name, serviceDescription, emailTone, businessIds, individualRecipients } = data;

    try {
      // Calculate total recipients
      const businessCount = businessIds?.length || 0;
      const individualCount = individualRecipients?.length || 0;
      const totalRecipients = businessCount + individualCount;
      ...
      // Build campaign items array
      const campaignItems: Array<{
        campaign_id: number;
      ...
      // Add individual recipient items
